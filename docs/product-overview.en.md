...
- Mark mistakenly sent user messages as deleted on desktop and the full web interface. Keep struck-through text and submit one correction asking the agent to ignore the message and attachments and await further input. Stop running tasks first. Marks survive history sync and appear read-only on mobile; native history and completed actions are not undone.
...
- SVG file quick previews show the image by default and can switch to read-only source; line references open source first.
